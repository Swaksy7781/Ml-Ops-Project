...
from PricePredictor.pipeline.stage_02_handling_missing_values import MissingValueHandlingTrainingPipeline # Import new stage
...
STAGE_NAME = "Handling Missing Values stage" # Define stage name for new stage
try:
    logger.info(f">>>>>> stage {STAGE_NAME} started <<<<<<")
    obj = MissingValueHandlingTrainingPipeline() # Instantiate and run new stage
    obj.main()
    logger.info(f">>>>>> stage {STAGE_NAME} completed <<<<<<\n\nx==========x")
except Exception as e:
    logger.exception(e)
